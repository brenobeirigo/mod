...
class ActionSpace(object):
    ...
    def get_action(self, cars, level):
        ...
        for c in cars:
            ...
            actions[
                (
                    (c.previous.id_level(level), c.previous_battery_level),
                    decision,
                )
            ] += 1
    ...
    def read_single_action_tuple(action):
        ...
        decision_category = (
            "servicing/staying"
            if action_code == Amod.TRIP_STAY_DECISION
            else "recharging/rebalancing"
        )
...
class Adp:
    def print_dimension(self, max_number_trips, total_time_steps):
        ...
        cars_per_attribute = scipy.misc.comb(
            car_attributes + len(self.mod.cars) - 1, len(self.mod.cars)
        )
        ...
        trips_per_attribute = scipy.misc.comb(
            trip_attributes + max_number_trips - 1, max_number_trips
        )
        ...
        print(
            ...
            f"\n - States in episode ({total_time_steps} steps):"
            f" {physical_state_count * total_time_steps}"
        )

    def iterate(self, env, trips, step, epsilon=0.1, gamma=0.8, alpha=0.1):
        ...
        reward, list_serviced, list_rejected = fcfs(self.mod, trips, step)
    ...
    def __init__(self, env):
        ...
        self.state_count = [
            defaultdict(int) for g in range(self.mod.config.aggregation_levels)
        ]
    ...
    def stats(self):
        ...
        for g, state_dic in enumerate(self.state_count):
            ...
            print(
                f"# Aggregation level {g} - mean visit = {total_count:>4.2f}"
            )
        ...
        for g, table in enumerate(self.table):
            ...
            for state, action_dict in table.items():
                ...
                if len(action_dict.keys()) > 1:
                    ...
                    print(
                        f" - [{state[0]}] (len={len(str(state))}) - #actions={len(action_dict.keys())} - action values={action_dict.values()}"
                    )
